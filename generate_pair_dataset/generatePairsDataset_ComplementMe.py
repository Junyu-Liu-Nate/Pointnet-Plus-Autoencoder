...
def generatePairsDataset(outputPath):
    ...
    for idx in range(len(datasetTypes)):
        ...
        # featureDataPath = os.path.join(PROJECT_PATH, "generated", "ComplementMe", "02691156", dataTypes[idx])
        featureDataPath = os.path.join(DATASET_PATH, "ComplementMe", "parts", category, dataTypes[idx])
        instanceNames = [d for d in os.listdir(featureDataPath) if os.path.isdir(os.path.join(featureDataPath, d))]

        pos_pair_counter = 0
        neg_pair_counter = 0
        for instanceName in instanceNames:
            if not instanceName in selectedWholeNames:
                continue
...
def get_all_pointnet_features(selectedWholeNames):
    ...
    featureDataPath = os.path.join(PROJECT_PATH, "generated", "ShapeNet", "autoencoder_airplane_ShapeNetv2.txt")
...
def main():
    ...
    outputName = "pairs_airplane_ComplementMe_decompose_resample_exact_oct_autoencoder"
