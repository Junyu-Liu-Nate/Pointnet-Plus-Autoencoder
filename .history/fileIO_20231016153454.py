...
# For ShapeNet
def readWholePointnetFeatures_ShapeNet(folder_path, selectedWholeNames):
    # shapeNames = [f for f in os.listdir(folder_path) if f.endswith('.npy')]
    whole_features = {}

    for shapeName in selectedWholeNames:
        file_name = shapeName
        file_path = os.path.join(folder_path, file_name + ".npy")
        wholeFeature = np.load(file_path)
        whole_features[shapeName] = wholeFeature

    return whole_features

def readSelectedMD5s(filename):
    with open(filename, 'r') as f:
        lines = f.readlines()
