...
def inferenceFeature(model, meshPaths):
    pointClouds = []
    for meshPath in meshPaths:
        pointCloud = read_off(meshPath)
        pointClouds.append(pointCloud)
    point_cloud = torch.tensor(pointClouds, dtype=torch.float32).unsqueeze(0).permute(0, 2, 1)  # Convert to tensor and adjust dimensions
