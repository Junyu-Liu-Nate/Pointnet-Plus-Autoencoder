...
def main():
    ...
    outputName = "ShapeNetv2_Wholes_100"
    objectName = "airplane"
    ...
    for i in range(len(pcPaths)):
        ...
        numParts = 100
        wholeFeature = computeWholeFeature(outputName, pcNames[i][:], model, wholeVertices, numParts, 1024)

        savePath = os.path.join(outputPath, pcNames[i])
        saveWholeFeature(wholeFeature, savePath)

        print(f"Finish generating whole feature for idx: {pcNames[i]}.")
