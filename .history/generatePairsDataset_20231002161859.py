...
def generatePairsDataset(outputPath, wholeMeshPaths):
    ...
    for shapeIdx in range(len(wholeMeshPaths)):
        ...
        for partFeature in wholeFeature:
            ...
            print(f"    Positive index: {positive_whole_path_idxs} (remember to +1 when finding files)")
            ...
            for positive_whole_path_idx in positive_whole_path_idxs:
                pair = {
                    ...
                    'label': True,
                    'part': partFeature,
                    'whole': whole_mesh_paths[positive_whole_path_idx]
                }
            ...
            for negative_whole_path_idx in negative_whole_path_idxs:
                pair = {
                    ...
                    'label': False,
                    'part': part_feature,
                    'whole': whole_mesh_paths[negative_whole_path_idx]
                }
                add_pair_to_existing_dataset(dataset_path, pair)
                add_pair_to_existing_txt(dataset_path, pair)
                neg_pair_counter += 1
